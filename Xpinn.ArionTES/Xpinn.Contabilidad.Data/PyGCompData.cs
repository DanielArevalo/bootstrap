...
namespace Xpinn.Contabilidad.Data
{
    ...
    /// </summary>    
    public class PyGCompData : GlobalData
    {
        ...
        public List<PyGComp> ListarPyGComparativo(PyGComp pEntidad, Usuario vUsuario,int pOpcion)
        {
            ...
            List<PyGComp> lstPyGComp = new List<PyGComp>();
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        if (pOpcion == 1)
                            cmdTransaccionFactory.CommandText = "USP_XPINN_CON_PYGCOMP";
                        else //Niif
                            cmdTransaccionFactory.CommandText = "USP_XPINN_CON_PYGCOMPNIIF";
                        cmdTransaccionFactory.ExecuteNonQuery();
                    }
                    catch (Exception ex)
                    {
                        BOExcepcion.Throw("PyGCompCompData", "USP_XPINN_CON_PYGCOMP", ex);
                        return null;
                    }
                };
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = @"select fecha1, fecha2, fecha3, centro_costo_ini, centro_costo_fin, cod_cuenta, nombre_cuenta, nivel, sum(valor1) as valor1, sum(participacion1) as participacion1, sum(valor2) as valor2, sum(participacion2) As participacion2, sum(diferencia1) as diferencia1, porcentaje1/100 As porcentaje1, sum(valor3) as valor3, sum(participacion3) As participacion3, diferencia2, porcentaje2/100 As porcentaje2 
                                        from temp_balcomp group by fecha1, fecha2, fecha3, cod_cuenta, nombre_cuenta, centro_costo_ini, centro_costo_fin, nivel, porcentaje1, diferencia2, porcentaje2 
                                        order by cod_cuenta";
                    ...
                    {
                        BOExcepcion.Throw("PyGCompData", "ListarPyGComparativo", ex);
                        return null;
                    }
                }
            }
        ...
        {
            DbDataReader resultado = default(DbDataReader);
            List<PyGComp> lstFecha1 = new List<PyGComp>();
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(pUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    ...
                    {
                        BOExcepcion.Throw("PyGCompData", "ListarFecha1", ex);
                        return null;
                    }
                }
            }
