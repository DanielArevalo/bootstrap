...
public partial class Movimientos : GlobalWeb
{
    ...
    protected void btnConsultar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Page.IsValid)
            {
                Actualizar(ViewState["pProducto"].ToString(), ViewState["pTipoProduct"].ToString());
            }
        }
        catch (Exception ex)
        {
    ...
    private void Actualizar(string pProducto, string pTipoProduct)
    {
        try
        {
            DateTime pFec = DateTime.Now;
            pPersona = (xpinnWSLogin.Persona1)Session["persona"];
            if (string.IsNullOrEmpty(pTipoProduct))
                return;
            ...
            if (lstMov.Count > 0)
            {
                ...
                VerificarGridView(Convert.ToInt32(pTipoProduct));
            }
