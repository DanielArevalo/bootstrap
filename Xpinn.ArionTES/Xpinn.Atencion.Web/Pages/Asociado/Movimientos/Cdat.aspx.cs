...
public partial class Cdat : GlobalWeb
{
    ...
    private void ObtenerDatos(string pProducto)
    {
        try
        {
            ...
            txtFecIni.Text = DateTime.ParseExact(pFec.ToString("dd/MM/yyyy"), "dd/MM/yyyy", null).AddMonths(-24).ToString("dd/MM/yyyy");
            ...
            DateTime pFecIniProduct = DateTime.MinValue;
                    ...
                    try
                    {
                        //Obteniendo datos CDAT
                        List<xpinnWSDeposito.Cdat> lstCdat = new List<xpinnWSDeposito.Cdat>();
                        string pFiltroCdat = " AND C.ESTADO IN (1,2) AND T.COD_PERSONA = " + pPersona.cod_persona + " AND C.NUMERO_CDAT = " + pProducto;
                        ...
                        {
                            List<xpinnWSDeposito.Cdat> lstcdat = new List<xpinnWSDeposito.Cdat>();
                            xpinnWSDeposito.Cdat prog = lstCdat.ElementAt(0);
                            lstcdat.Add(prog);
                            frmVista.DataSource = lstcdat;
                            frmVista.DataBind();
                            frmVista.Visible = true;
                            ViewState["pProductoCodigo"] = lstcdat.ElementAt(0).codigo_cdat;
                        }
                        Actualizar(ViewState["pProductoCodigo"].ToString());
                    }
                    catch (Exception e)
                    {
    ...
    private void Actualizar(string pProducto)
    {
        try
        {
            ...
            List<xpinnWSEstadoCuenta.ReporteMovimiento> lstCdat = BOEstado.ListarMovCDAT(Convert.ToInt64(pProducto), pFecIni, pFecFin, Session["sec"].ToString());
            pGrilla.Visible = false;
            lblInfo.Visible = true;
            lblTotalReg.Visible = false;
            if (lstCdat != null && lstCdat.Count > 0)
            {
                pGrilla.Visible = true;
                lblInfo.Visible = false;
                lblTotalReg.Visible = true;
                lblTotalReg.Text = "<br/> Movimientos encontrados :" + lstCdat.Count;
                gvLista.DataSource = lstCdat;
                gvLista.DataBind();
            }
        }
        catch (Exception ex)
