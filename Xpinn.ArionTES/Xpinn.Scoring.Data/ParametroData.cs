...
namespace Xpinn.Scoring.Data
{
    ...
    public class ParametroData : GlobalData
    {
        ...
        {
            DbDataReader resultado = default(DbDataReader);
            List<Modelo> lstParametro = new List<Modelo>();
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(pUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        string sql = "SELECT * FROM vScCampos ORDER BY 1";
                        connection.Open();
                        cmdTransaccionFactory.Connection = connection;
                        cmdTransaccionFactory.CommandType = CommandType.Text;
                        cmdTransaccionFactory.CommandText = sql;
                        ...
                        while (resultado.Read())
                        {
                            ...
                            if (resultado["tbcol_name"] != DBNull.Value) entidad.Nombre = Convert.ToString(resultado["tbcol_name"]);
                            lstParametro.Add(entidad);
                        }
                        return lstParametro;
                    }
                    ...
                    {
                        BOExcepcion.Throw("ParametroData", "ListarCampos", ex);
                        return null;
                    }
                }
            }
