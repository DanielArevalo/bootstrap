...
{
    public class PaymentACHData : GlobalData
    {
        protected ConnectionDataBase dbConnectionFactory;

        public PaymentACHData()
        {
            dbConnectionFactory = new ConnectionDataBase();
        }

        public PaymentACH CreateModifyPaymentACH(PaymentACH pPaymentACH, ref string pError, OperacionCRUD pOperacion, Usuario vUsuario)
        {
        ...
        public PaymentACH ConsultPaymentACH(string pFiltro, ref string pError, Usuario vUsuario)
        {
            DbDataReader resultado;
            PaymentACH entidad = null;
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
        ...
        public List<PaymentACH> ListPaymentACH(string pFiltro, ref string pError, Usuario vUsuario)
        {
            DbDataReader resultado;
            List<PaymentACH> lstPayments = null;
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        if (resultado.HasRows)
                        {
                            lstPayments = new List<PaymentACH>();
                            PaymentACH entidad;
