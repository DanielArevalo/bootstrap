...
namespace Xpinn.Tesoreria.Data
{
    ...
    /// </summary>
    public class ArqueoPagosData : GlobalData
    {
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = @"Select A.Cod_Ope, Op.Cod_Ofi Coficina, D.Nombre NomOficina, Op.Cod_Cajero ccajero,X.Nombre ccaja, T.Descripcion as nommoneda, "
                                            +"op.fecha_oper fechamov, a.cod_persona codpersona,  p.nombre nom_cliente, "
                                            ...
                                            +"From movimientocaja a Inner Join operacion op On op.cod_ope = a.cod_ope "
                        ...
                        if (pFechaIni != DateTime.MinValue && pFechaIni != null)
                        {
                            if (dbConnectionFactory.TipoConexion() == "ORACLE")
                                sql += " and op.fecha_oper >= To_Date('" + Convert.ToDateTime(pFechaIni).ToString(conf.ObtenerFormatoFecha()) + "', '" + conf.ObtenerFormatoFecha() + "')";
                            else
                        ...
                        {
                            if (dbConnectionFactory.TipoConexion() == "ORACLE")
                                sql += " and op.fecha_oper <= To_Date('" + Convert.ToDateTime(pFechaFin).ToString(conf.ObtenerFormatoFecha()) + "', '" + conf.ObtenerFormatoFecha() + "')";
                            else
