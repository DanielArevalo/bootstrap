...
{
    Xpinn.Riesgo.Services.HistoricoSegmentacionService _historicoService = new Xpinn.Riesgo.Services.HistoricoSegmentacionService();
    ...
    protected void Page_PreInit(object sender, System.EventArgs e)
    {
        try
        {
            VisualizarOpciones(_historicoService.CodigoPrograma, "L");
    ...
    private void LlenarCombos()
    {
        ...
        List<Cierea> lstFechaCierre = _historicoService.ListarFechaCierre1("X", Usuario); //.ListarFechaCierre(Usuario);
        List<HistoricoSegmentacion> lstFechasYaCerradas = _historicoService.ListarFechaCierreYaHechas("D", Usuario);
        ...
        if (lstFechasYaCerradas != null && lstFechasYaCerradas.Count > 0)
        {
            ...
            lstFechaCierre = lstFechaCierre.Where(x =>
            {
                ...
                foreach (var fechaCerrada in lstFechasYaCerradas)
                {
                    ...
                    if (fechaCerrada.fechacierre.Day == dia && fechaCerrada.fechacierre.Month == mes && fechaCerrada.fechacierre.Year == año)
                    {
                        return false;
                    }
                }
    ...
    public void TerminarProceso()
    {
        ...
        if (Session["Error"] != null)
        {
            ...
            {
                Label2.Text = "Proceso de Segmentaciòn de Personas Terminado Correctamente";
            }
            else
            {
    ...
    public void EjecutaProceso()
    {
        ...
        try
        {
            _historicoService.CierreSegmentacion(_fecha, _estado, Usuario);
        }
        catch (Exception ex)
        {
            sError = ex.Message;
