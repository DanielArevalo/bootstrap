...
public partial class Lista : GlobalWeb
{
    ...
    protected void Page_PreInit(object sender, EventArgs e)
    {
        try
        {
            ...
            toolBar.eventoNuevo += (s, evt) => {
                Session.Remove(_liquidacionServices.CodigoProgramaanticipos + ".id");
    ...
    protected void gvLista_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Session[_liquidacionServices.CodigoProgramaanticipos + ".id"] = id;
        Navegar(Pagina.Detalle);
    }
    ...
    void CtlMensajeBorrar_eventoClick(object sender, EventArgs e)
    {
        if (ViewState["idBorrar"] != null)
        {
            ...
            try
            {
                _liquidacionServices.EliminarLiquidacionNomina(idBorrar, Usuario);
                Actualizar();
            }
            catch (Exception ex)
            {
    ...
    void Actualizar()
    {
        try
        {
            ...
            List<LiquidacionNomina> lstLiquidacion = _liquidacionServices.ListarAnticiposNomina(filtro, Usuario);
    ...
    string ObtenerFiltro()
    {
        ...
        {
            filtro += " and an.consecutivo = " + txtCodigoLiquidacion.Text.Trim();
        }
        
        if (!string.IsNullOrWhiteSpace(txtFechaInicio.Text) && !string.IsNullOrWhiteSpace(txtFechaFin.Text))
        {
            filtro += " and an.FECHAINICIO = to_Date('" + txtFechaInicio.Text.Trim() + "', 'dd/MM/yyyy') and an.FECHATERMINACION = to_Date('" + txtFechaFin.Text.Trim() + "', 'dd/MM/yyyy') ";
        }
        else if (!string.IsNullOrWhiteSpace(txtFechaInicio.Text))
        {
            filtro += " and an.FECHAINICIO = to_Date('" + txtFechaInicio.Text.Trim() + "', 'dd/MM/yyyy') ";
        }
        else if(!string.IsNullOrWhiteSpace(txtFechaFin.Text))
        {
            filtro += " and an.FECHATERMINACION = to_Date('" + txtFechaFin.Text.Trim() + "', 'dd/MM/yyyy') ";
        }

        if (!string.IsNullOrWhiteSpace(ddlCentroCosto.SelectedValue))
        {
            filtro += " and an.CODIGOCENTROCOSTO = " + ddlCentroCosto.SelectedValue;
        }

        if (!string.IsNullOrWhiteSpace(ddlTipoNomina.SelectedValue))
        {
            filtro += " and an.CODIGONOMINA = " + ddlTipoNomina.SelectedValue;
        }
        filtro += " and an.tipo =  0";
