...
public partial class Detalle : GlobalWeb
{
    ...
    protected void gvfam_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "Delete")
        {
            ...
            object dataKey = gvfam.DataKeys[index].Value;

            if (dataKey != null && !string.IsNullOrWhiteSpace(dataKey as string))
            {
                long consecutivo = Convert.ToInt64(dataKey);
    ...
    {
        Page.Validate();
        if (!Page.IsValid || string.IsNullOrWhiteSpace(txtIdentificacionE.Text) || string.IsNullOrWhiteSpace(txtFechaexpedicion.Text) || string.IsNullOrWhiteSpace(txtDireccionE.Text)
    ...
    void GuardarDatos()
    {
        try
        {
            ...
            //Información Adicional adicional de empleado que no esta en persona
            if (empleado.consecutivo> 0)
            {
                empleado = _empleadoService.CrearPersonaEmpleados(empleado, Usuario);
            }
            else
            {
                empleado = _empleadoService.ModificarPersonaEmpleados(empleado, Usuario);
            }
    ...
    private Empleados ObtenerEntidadGuardar()
    {
       ...
       // empleado.tipo_sangre = ddlTipoSangre.SelectedValue;       
        ///empleado.visa = (rblVisa.Text != "") ? Convert.ToString(rblVisa.SelectedValue) : String.Empty;
        //empleado.pasaporte = (rblPasaporte.Text != "") ? Convert.ToString(rblPasaporte.SelectedValue) : String.Empty;
        //empleado.fecha_ven_visa = !string.IsNullOrWhiteSpace(txtFechavencvisa.Text) ? Convert.ToDateTime(txtFechavencvisa.Text) : default(DateTime?);
        //empleado.fecha_ven_pasaporte = !string.IsNullOrWhiteSpace(txtFechavencpasaporte.Text) ? Convert.ToDateTime(txtFechavencpasaporte.Text) : default(DateTime?);
    ...
    List<Empleado_Familiar> ObtenerFamiliares()
    {
        ...
        foreach (GridViewRow row in gvfam.Rows)
        {
            ...
            int consecutivo = Convert.ToInt32(gvfam.DataKeys[row.RowIndex].Value.ToString());
