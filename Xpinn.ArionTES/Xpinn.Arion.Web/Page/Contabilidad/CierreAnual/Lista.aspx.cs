...
{
    private Xpinn.Contabilidad.Services.CierreAnualService CierreAnualSer = new Xpinn.Contabilidad.Services.CierreAnualService();
    static ArrayList ar = new ArrayList();
    CierreAnual datosApp = new CierreAnual();
    ...
    protected void Page_PreInit(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                pCod_Programa = CierreAnualSer.CodigoPrograma;
                ViewState.Add("COD_PROGRAMA", "LOCAL");
            }
            else
            {
                if (Request.QueryString["niif"].ToString() == "1") //(Request.UrlReferrer.ToString().Contains("niif"))
                {
                    pCod_Programa = CierreAnualSer.CodigoProgramaNiif;
    ...
    private void Actualizar()
    {
        try
        {
            ...
            {
                mvCierreAnual.ActiveViewIndex = 0;
                gvLista.DataBind();
            }
            else
            {
                mvCierreAnual.ActiveViewIndex = -1;
            }
    ...
    private void Guardar(String pIdObjeto)
    {
        try
        {
            ...
            try
            {
                ...
                if (isNiif)
                {
                    if (ciereaServicio.ExisteCierre(datosApp.fecha, "O", Usuario))
                    {
                        VerError("Ya se hizo el cierre anual NIIF para la fecha dada");
                        return;
                    }
                }
                else
                { 
                    if (ciereaServicio.ExisteCierre(datosApp.fecha, "N", Usuario))
                    {
                        VerError("Ya se hizo el cierre anual para la fecha dada");
                        return;
                    }
                }
            }
                ...
                try
            {
                CierreAnual cierreAnual = new CierreAnual();
                cierreAnual = CierreAnualSer.CrearCierreAnual(datosApp, ref error, isNiif, Usuario);                
            }
            catch (Exception ex)
            {
                int n = 1;
            ...
            foreach (GridViewRow wrow in gvLista.Rows)
            {
                ...
                lblmensaje.Text = "Cierre Anual Terminado Correctamente";                  
            }
            Site toolBar = (Site)this.Master;
