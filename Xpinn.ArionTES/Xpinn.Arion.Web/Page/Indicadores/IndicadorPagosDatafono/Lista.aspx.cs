...
partial class Lista : GlobalWeb
{
    ...
    protected void Page_PreInit(object sender, System.EventArgs e)
    {
        try
        {
            VisualizarOpciones(CarteraCategoriasServicio.CodigoProgramaDatafono, "L");
            Site toolBar = (Site)this.Master;
            txtColorFondo.eventoCambiar += txtColorFondo_TextChanged;
        }
        catch (Exception ex)
        {
            BOexcepcion.Throw(CarteraCategoriasServicio.CodigoProgramaDatafono, "Page_PreInit", ex);
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            if (Session[CarteraCategoriasServicio.CodigoProgramaDatafono + ".id"] != null)
            {
    ...
    protected void btnInforme_Click(object sender, EventArgs e)
    {
        ...
        dtDatos.Columns.Add("numero", typeof(decimal));
        
        List<IndicadorCartera> LstDetalleComprobante = new List<IndicadorCartera>();
        LstDetalleComprobante = CarteraCategoriasServicio.IngresosDatafono(pFechaIni, pFechaFin, (Usuario)Session["Usuario"]);
        Decimal total = 0;
        Decimal totalnumero = 0;
        //llenando la tabla
        foreach (IndicadorCartera rFila in LstDetalleComprobante)
        {
            ...
            if (rFila.nombre != null)
            {
                ...
                drDatos[2] = rFila.numero;
                total += rFila.valor;
                totalnumero += rFila.numero;
                dtDatos.Rows.Add(drDatos);
            }
        }
        DataRow drTotal;
        drTotal = dtDatos.NewRow();
        drTotal[0] = "TOTAL TARJETAS";
        drTotal[1] = total;
        drTotal[2] = totalnumero;
        dtDatos.Rows.Add(drTotal);
        ...
        Chart1.Titles["Title1"].Text = "INGRESOS POR DATAFONOS";
