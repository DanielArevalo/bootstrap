...
namespace Xpinn.Reporteador.Data
{
    public class ExtractoData : GlobalData
    {
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = @" With suma As( select a.numero_aporte as numero_aporte, l.NOMBRE as Nom_linea,
                                            ESNULO((Select saldo From historico_aporte hist Where hist.FECHA_HISTORICO = (SELECT MIN(histAN.FECHA_HISTORICO) FROM historico_aporte histAN WHERE histAN.FECHA_HISTORICO BETWEEN to_date('" + fechaInicial.AddDays(-1).ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy')
                                                    And histAN.NUMERO_APORTE = a.NUMERO_APORTE) and hist.NUMERO_APORTE = a.NUMERO_APORTE), 0) as Saldo_Inicial, 
                                            ESNULO((Select Saldo From Historico_Aporte Hist Where Hist.Fecha_Historico = To_Date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') And hist.NUMERO_APORTE = a.NUMERO_APORTE), 0) as Saldo_Final, 
                                            ESNULO((Select sum(valor) from tran_aporte ta inner  join operacion o on o.cod_ope = ta.cod_ope where ta.tipo_tran = 104 and o.estado = 1 and o.fecha_oper BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and ta.NUMERO_APORTE = a.NUMERO_APORTE), 0) as Retencion,
                                            ESNULO((Select sum(valor) from tran_aporte ta inner  join operacion o on o.cod_ope = ta.cod_ope where ta.tipo_tran In (103, 111,99) and o.estado = 1 and o.fecha_oper BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and ta.NUMERO_APORTE = a.NUMERO_APORTE), 0) as Intereses,
                                            ESNULO((select sum(valor) from tran_aporte ta1 inner join operacion o on o.cod_ope = ta1.cod_ope where ta1.tipo_tran In (select tipo_tran from tipo_tran where tipo_producto = 1 and tipo_mov = 2 and tipo_tran Not In (103, 111,99)) And o.fecha_oper BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') And ta1.NUMERO_APORTE = a.NUMERO_APORTE), 0) as MovimientosCreditos,
                                            ESNULO((select sum(valor) from tran_aporte ta2 inner join operacion o on o.cod_ope = ta2.cod_ope where ta2.tipo_tran In (select  tipo_tran from tipo_tran where tipo_producto = 1 and tipo_mov= 1 and tipo_tran Not In (103, 111))  and o.fecha_oper BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') And ta2.NUMERO_APORTE = a.NUMERO_APORTE), 0) as MovimientosDebitos
                                            From aporte a
                                            ...
                                            Where a.COD_PERSONA = " + cod_persona + " And h.FECHA_HISTORICO BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') And Nvl(l.nomostrar_extracto, 0) != 1 
                                            Group by  a.numero_aporte, l.nombre)
                                            select numero_aporte, Nom_linea, Saldo_Inicial, Saldo_Final, Intereses, MovimientosCreditos, MovimientosDebitos,Retencion From suma ";
                        ...
                        while (resultado.Read())
                        {
                            ...
                            if (entidad.saldo_inicial > 0 || entidad.movimientoD > 0 || entidad.movimientoC > 0)
                                lstExtracto.Add(entidad);
                        }
                        return lstExtracto;
                    }
        ...
        public decimal ValorRevalorizacion(int cod_persona, DateTime fechaInicial, DateTime fechaFinal, Usuario usuario)
        {
            decimal entidad = 0;
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(usuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = @"Select Sum(hist.saldo) As saldo From historico_aporte hist Join aporte apo On hist.numero_aporte = apo.numero_aporte Join lineaaporte lin On apo.cod_linea_aporte = lin.cod_linea_aporte
                                        Where hist.FECHA_HISTORICO = (SELECT MAX(histAN.FECHA_HISTORICO) From historico_aporte histAN Where histAN.FECHA_HISTORICO BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy')  AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') And histAN.NUMERO_APORTE = apo.NUMERO_APORTE) 
                                        And apo.cod_persona = " + cod_persona + " And Upper(lin.nombre) Like '%REVALORIZACION%' ";
                        ...
                        if (resultado.Read())
                        {
                            if (resultado["saldo"] != DBNull.Value) entidad = Convert.ToInt64(resultado["saldo"]);
                        }
                        return entidad;
                    }
                    catch (Exception ex)
                    {
                        BOExcepcion.Throw("ExtractoData", "ValorRevalorizacion", ex);
                        return entidad;
                    }
                }
            }
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = @"select  aho.numero_cuenta, lin.DESCRIPCION as NOM_LINEA, histo.NUMERO_CUENTA,
                                          Round(ESNULO((select hist.SALDO_TOTAL from historico_ahorro hist where hist.FECHA_HISTORICO = to_date('" + fechaInicial.AddDays(-1).ToShortDateString() + @"', 'dd/MM/yyyy') And hist.NUMERO_CUENTA = histo.NUMERO_CUENTA),0),0) as Saldo_Inicial,
                                          Round(ESNULO((select hist.SALDO_TOTAL from historico_ahorro hist where hist.FECHA_HISTORICO = to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') And hist.NUMERO_CUENTA = histo.NUMERO_CUENTA),0),0) as Saldo_Final,
                                          Round(ESNULO((select SUM(tr.VALOR) from operacion o join tran_ahorro tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN  where o.FECHA_OPER BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and tip.TIPO_MOV = 2 and tr.ESTADO != 2 and tr.TIPO_TRAN NOT IN (207,208,252) and tr.NUMERO_CUENTA = histo.NUMERO_CUENTA and tr.cod_ope not in(select cod_ope from operacion_anulada)),0),0) as MovimientosCreditos,
                                          Round(ESNULO((select SUM(tr.VALOR) from operacion o join tran_ahorro tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN where o.FECHA_OPER BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and tip.TIPO_MOV = 1 and tr.ESTADO != 2 and tr.TIPO_TRAN NOT IN (207,208,252) and tr.NUMERO_CUENTA = histo.NUMERO_CUENTA and tr.cod_ope not in(select cod_ope from operacion_anulada)),0),0) as MovimientosDebitos,
                                          Round(ESNULO((select SUM(tr.VALOR) from operacion o join tran_ahorro tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN where o.FECHA_OPER BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and tr.TIPO_TRAN in( 207,252)  and o.estado=1 and tr.NUMERO_CUENTA = histo.NUMERO_CUENTA),0),0) as Interes,
                                          Round(ESNULO((select SUM(tr.VALOR) from operacion o join tran_ahorro tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN where o.FECHA_OPER BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and tr.TIPO_TRAN = 208  and o.estado=1  and tr.NUMERO_CUENTA = histo.NUMERO_CUENTA),0),0) as Retencion,
                                          Round(ESNULO((select SUM(tr.VALOR) from operacion o join tran_ahorro tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN where o.FECHA_OPER BETWEEN to_date('" + fechaInicial.ToShortDateString() + "', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and tip.TIPO_MOV = 2 and tr.TIPO_TRAN = 203 and tr.NUMERO_CUENTA = histo.NUMERO_CUENTA),0),0) as Abonos
                        ...
                        while (resultado.Read())
                        {
                            ...
                            if (resultado["MovimientosDebitos"] != DBNull.Value) entidad.movimientoD = Convert.ToDecimal(resultado["MovimientosDebitos"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig()));
                            if (entidad.movimientoC > 0 || entidad.movimientoD > 0 || entidad.saldo_inicial > 0)
                                lstExtracto.Add(entidad);
                        }
                        return lstExtracto;
                    }
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(usuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql =
                        @"With sums as (
                            Select histo.numero_radicacion, MIN(l.nombre) as nom_linea,
                            ...
                            ESNULO(NVL((select hist.SALDO_CAPITAL from historico_cre hist where hist.FECHA_HISTORICO = (SELECT MIN(histAN.FECHA_HISTORICO) FROM historico_cre histAN WHERE histAN.FECHA_HISTORICO = to_date('" + fechaInicial.AddDays(-1).ToShortDateString() + "', 'dd/MM/yyyy')" + @" and histAN.NUMERO_RADICACION = histo.NUMERO_RADICACION) and hist.NUMERO_RADICACION = histo.NUMERO_RADICACION), 0),0) - 
                            ESNULO(NVL((select hist.SALDO_CAPITAL from historico_cre hist where hist.FECHA_HISTORICO = (SELECT MAX(histAN.FECHA_HISTORICO) FROM historico_cre histAN WHERE histAN.FECHA_HISTORICO = to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy')  and histAN.NUMERO_RADICACION = histo.NUMERO_RADICACION) and hist.NUMERO_RADICACION = histo.NUMERO_RADICACION), 0),0)  as Saldo_pagado
                            From HISTORICO_CRE histo Join LINEASCREDITO l on l.cod_linea_credito = histo.cod_linea_credito
                            ...
                            Group By Histo.numero_radicacion
                        Order by 1) Select * from Sums
                        ";
                        connection.Open();
                        cmdTransaccionFactory.Connection = connection;
                        cmdTransaccionFactory.CommandType = CommandType.Text;
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(usuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        ESNULO((select hist.valor from historico_cdat hist where hist.FECHA_HISTORICO = to_date('" + fechaInicial.ToShortDateString() + @"', 'dd/MM/yyyy') And hist.numero_cdat = histo.numero_cdat),0) as Saldo_Final,                                                       
                        ...
                        ESNULO((select sum(t.VALOR) from tran_cdat t join operacion o on t.cod_ope = o.cod_ope where o.fecha_oper BETWEEN to_date('" + fechaInicial.ToShortDateString() + @"', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and t.tipo_tran In (301, 302) and o.estado=1 and t.codigo_cdat = c.codigo_cdat), 0) as apertura,
                        ESNULO((select sum(t.VALOR) from tran_cdat t join operacion o on t.cod_ope = o.cod_ope where o.fecha_oper BETWEEN to_date('" + fechaInicial.ToShortDateString() + @"', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and t.tipo_tran In (305) and o.estado=1 and t.codigo_cdat = c.codigo_cdat), 0) as Movimiento,
                        ...
                        while (resultado.Read())
                        {
                            ...
                            if (resultado["Saldo_Inicial"] != DBNull.Value) entidad.saldo_inicial = Convert.ToDecimal(resultado["Saldo_Inicial"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig()));
                            if (resultado["Saldo_Final"] != DBNull.Value) entidad.saldo_final = Convert.ToDecimal(resultado["Saldo_Final"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig()));
                            if (resultado["Movimiento"] != DBNull.Value) entidad.movimientoD = Convert.ToDecimal(resultado["Movimiento"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig())); ;
                            if (resultado["Intereses"] != DBNull.Value) entidad.interes_corriente = Convert.ToDecimal(resultado["Intereses"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig()));
                            if (resultado["Retencion"] != DBNull.Value) entidad.retencion = Convert.ToDecimal(resultado["Retencion"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig()));
                            if (resultado["apertura"] != DBNull.Value) entidad.movimientoC = Convert.ToDecimal(resultado["apertura"].ToString().Replace(".", conf.ObtenerSeparadorDecimalConfig()));
                            ...
                            //}
                            entidad.saldo_final = entidad.saldo_inicial + entidad.movimientoC - entidad.movimientoD;
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = @"select aho.numero_programado as numero_programado, lin.NOMBRE as NOM_LINEA, histo.numero_programado,
                                        ...
                                        FROM historico_programado histAN WHERE histAN.FECHA_HISTORICO = to_date('" + fechaInicial.AddDays(-1).ToShortDateString() + @"', 'dd/MM/yyyy') and histAN.numero_programado = histo.numero_programado) and hist.numero_programado = histo.numero_programado),0) as Saldo_Inicial,
                                        ...
                                        ESNULO((select SUM(tr.VALOR) from operacion o join tran_programado tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN 
                                        where o.FECHA_OPER BETWEEN  to_date('" + fechaInicial.ToShortDateString() + @"', 'dd/MM/yyyy') AND to_date('" + fechaFinal.ToShortDateString() + @"', 'dd/MM/yyyy') and tr.TIPO_TRAN in(353,355)  and o.estado=1  and tr.numero_programado = histo.numero_programado),0) as Interes,
                                        ESNULO((select SUM(tr.VALOR) from operacion o join tran_programado tr on tr.COD_OPE = o.COD_OPE join tipo_tran tip on tr.TIPO_TRAN = tip.TIPO_TRAN 
                        ...
                        while (resultado.Read())
                        {
                            ...
                            if ( entidad.movimientoD > 0 || entidad.movimientoC > 0)
                                lstExtracto.Add(entidad);
                        }
                        return lstExtracto;
                    }
        ...
        {
            DbDataReader resultado = default(DbDataReader);
            List<Persona1Ext> lstPersona1 = new List<Persona1Ext>();
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(usuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        while (resultado.Read())
                        {
                            Persona1Ext entidad = new Persona1Ext();
